...
THIRD-PARTY IMAGES ARE PINNED HERE TOO, and `postgres` is why. It was pulled as
`postgres:${POSTGRES_VERSION:-16}` -- a rolling MAJOR, so any patch or minor
Postgres published upstream silently became the database this stack was tested
against, and a `:-16` default meant versions.env was not even required to have
an opinion. `unitycatalog` is pinned for the same reason at a finer tag.
"""
import re
import subprocess
